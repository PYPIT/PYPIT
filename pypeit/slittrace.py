...
class SlitTraceSet(datamodel.DataContainer):
    ...
    def assign_maskinfo(self, sobjs, plate_scale, slits_left, slits_right, det_buffer, TOLER=1.):
        ...
        # Brightest object in this detector (if it exists)
        brightest = (obj_mag == obj_mag.min()) & (obj_mag > 13) & (obj_mag <= 20) & \
                    (self.maskdef_designtab['ALIGN'].data == 0)
        bright_objname = self.maskdef_designtab['OBJNAME'].data[brightest]
        ...
        if sobjs.nobj == 0:
            ...
            for maskid in uni_maskid:
                ...
                if np.any(in_toler):
                    ...
                    # if this object is the brightest object, record the offset from the expected location
                    if (len(bright_objname) > 0) & (sobj.MASKDEF_OBJNAME == bright_objname):
                        median_off = expected[imx_idx]-measured[imx_idx]
                        msgs.info('Bright OBJNAME: {}, median_offset={}'.format(bright_objname, median_off))
